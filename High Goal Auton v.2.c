...
task autonomous()
{
	motor[Lift1_L]=motor[Lift1_R]=motor[RaiseR]=motor[RaiseL]=127;
	wait(1);
	motor[Lift1_L]=motor[Lift1_R]=motor[RaiseR]=motor[RaiseL]=-0;
// The lift and raise moves up 1 second
	nMotorEncoder[Left]= 0;
	nMotorEncoder[Right]= 0;
	while(abs(nMotorEncoder[Left]) < 458.73)
	while(abs(nMotorEncoder[Right]) < 458.73)
	{
...
//Robot goes forward for 17 inches
	nMotorEncoder[Left]= 0;
	nMotorEncoder[Right]= 0;
	while(abs(nMotorEncoder[Left]) < 1215.2)
	{
		motor[Left]=127;
	}
	motor[Left]=0;
//Robot makes a 90 degree swing turn to the right
	nMotorEncoder[Left]= 0;
	nMotorEncoder[Right]= 0;
	while(abs(nMotorEncoder[Left]) < 1307.22)
	while(abs(nMotorEncoder[Right]) < 1307.22)
	{
		motor[Left]=motor[Right]=127;
	}
	motor[Left]=motor[Right]=0;
//Robot goes forward to high goal
...
//Im not sure if i need this ^ to score the high goal or not so I just commented it out
	motor[Claw]= 127;
	wait(0.5);
//Opens claw and drops cone on high goal
	nMotorEncoder[Left]= 0;
	nMotorEncoder[Right]= 0;
	while(abs(nMotorEncoder[Left]) < 1307.22)
	while(abs(nMotorEncoder[Right]) < 1307.22)
